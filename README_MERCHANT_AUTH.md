...
API-ключ хранится не в открытом виде, а как SHA-256-хеш. Secret key нужен для проверки HMAC, поэтому его нельзя просто захешировать. В PCI DSS-подобном режиме его можно хранить в PostgreSQL в виде `vault:v1:...` через HashiCorp Vault Transit.

## Роли доступа

В прототипе поддерживаются три роли:

```text
merchant — обычный интернет-магазин, видит только свои платежи и свою статистику;
admin    — внутренняя административная роль, может смотреть статистику любого мерчанта.
auditor  — внутренняя read-only роль, может смотреть статусы, возвраты и отчёты, но не создавать платежи и возвраты.
```

Роль хранится в `merchants.role`. Для демо-режима без PostgreSQL можно задать:

```powershell
$env:MERCHANT_ROLE="admin"
```

Для обычного интернет-магазина оставляй значение по умолчанию `merchant`. Неизвестная роль отклоняется при аутентификации, то есть доступ запрещается по умолчанию.
